...
class Lynk:
    ...
    def create_database(self, name: str = "lynkio_test_db",
                        ...
                        auto_sync_log: bool = False):
        """Create a soketDB instance and optionally set up log tables."""
        if not self.enable_database:
            raise Exception("Database is not enabled.")

        config = self.database_config or {
        ...
        try:
            ...
            if db:
                self._db = db
                self.auto_sync_log = auto_sync_log
                _databases[name] = db          # store globally for distributed queries

                if create_log_table:
                    # Create the three log tables (soketDB: only column names, no types)
                    db.execute("""
                        CREATE TABLE IF NOT EXISTS wss_logs (
                            id, timestamp, client_id, direction, event, data, size, opcode
                        )
                    """)
                    db.execute("""
                        CREATE TABLE IF NOT EXISTS http_logs (
                            id, timestamp, method, path, status_code, client_ip, user_agent, response_time, request_id
                        )
                    """)
                    db.execute("""
    ...
    async def _log_websocket(self, client_id: str, direction: str,
                             ...
                             opcode: int = None, size: int = 0):
        """Log a WebSocket message/event if auto_sync_log is True."""
        if not self._db or not self.auto_sync_log:
            return
        data_str = None
        if data is not None:
    ...
    async def emit(self, event: str, data: Any, client_id: Optional[str] = None) -> None:
        ...
        if client_id is not None:
            ...
            if client and not client.closed:
                ...
                await self._log_websocket(client_id, 'out', event=event,
    ...
    async def emit_to_all_except(self, event: str, data: Any, exclude_ids: List[str]) -> None:
        ...
        for cid, c in self._clients.items():
            if not c.closed and cid not in exclude_ids:
                tasks.append(c.send(message))
                # Log each outgoing message (could be many; consider rate‑limiting)
                await self._log_websocket(cid, 'out', event=event,
                                          data=data, size=len(message))
        if tasks:
        ...
        try:
            ...
            for mw in self._http_middleware:
                try:
                    ...
                    await self._send_http_error(writer, 500, "Middleware error", req)
                    return
            ...
            for pattern, handler, methods in self._http_routes:
                ...
                if match:
                    if method not in methods:
                        await self._send_http_error(writer, 405, f"Method {method} not allowed", req)
                        return
                    ...
                    try:
                        ...
                        await self._send_http_error(writer, e.status_code, e.message, req)
                    except Exception as e:
                ...
                await self._send_http_error(writer, 404, f"Not Found: {route_path}", req)
    ...
    async def _websocket_handshake(
        ...
        await self._log_websocket(client.id, 'in', event='connect')
